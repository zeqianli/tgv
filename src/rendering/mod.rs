        ...
        match self {
            LayoutNode::Split {
                ...
                for child in children.iter() {
                    ...
                    child.populate_retrival_paths(&mut paths)?;
                    for (i, path) in paths.iter_mut() {
                        if *i == child.id() {
...
impl MainLayout {
    ...
    fn get_node(&self, node_id: usize) -> Result<&LayoutNode, TGVError> {
        let path = self
            .retrival_paths
            .get(&node_id)
    ...
    fn get_node_mut(&mut self, node_id: usize) -> Result<&mut LayoutNode, TGVError> {
        let path = self
            .retrival_paths
            .get(&node_id)
            .ok_or(TGVError::StateError(format!("Node not found: {}", node_id)))?
        ...
        match &state.display_mode {
            DisplayMode::Main => {
                ...
                for (area_id, rect) in area_lookup.iter() {
                    let node = state.layout.get_node(*area_id)?;
                    match node {
                        LayoutNode::Area { id: _, area_type } => {
                            area_type.render(rect, buf, state, registers)?;
                        }
                        LayoutNode::Split {
                            id: _,
                            direction: _,
