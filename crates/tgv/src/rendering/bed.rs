...
pub fn render_bed(
    area: &Rect,
    buf: &mut Buffer,
    bed_intervals: &SortedIntervalCollection<BEDInterval>,
    alignment_view: &AlignmentView,
    pallete: &Palette,
) -> Result<(), TGVError> {
    let region = alignment_view.region(area);
    let intervals =
        bed_intervals.overlapping(region.contig_index(), region.start(), region.end())?; // FIXME: wasteful calculation here
